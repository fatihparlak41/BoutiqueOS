...
export default async function ReservationsPage({ searchParams }: { searchParams: Promise<{ durum?: string }> }) {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {rows.length === 0 ? <p className="border border-dashed border-line-strong px-4 py-8 text-center text-xs text-muted">{history ? "Geçmiş rezervasyon yok." : "Aktif rezervasyon yok."}</p> : (
