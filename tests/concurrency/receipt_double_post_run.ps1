<#
.SYNOPSIS
  BoutiqueOS Phase 7A - goods receipt double-post race (two real psql sessions).
...
$logA = Join-Path $res "concurrency_receipt_A.log"; $logB = Join-Path $res "concurrency_receipt_B.log"

Write-Host "`n==> setup (1 unit in stock, open register)" -ForegroundColor Cyan
& psql $conn -X -v ON_ERROR_STOP=1 -f (Join-Path $dir "20_receipt_setup.sql")
...
$procA = Start-Process -FilePath "psql" -ArgumentList @($conn, "-X", "-f", (Join-Path $dir "21_receipt_post_a.sql")) `
...
$outB = & psql $conn -X -f (Join-Path $dir "22_receipt_post_b.sql") 2>&1
...
$outV = & psql $conn -X -f (Join-Path $dir "23_receipt_verify.sql") 2>&1
$outV | ForEach-Object { Write-Host "    $_" }

$bFailedRight = ($outB | Select-String 'INVALID_STATE').Count -gt 0
$aOk          = ($outA | Select-String 'COMMIT done').Count -gt 0
$verifyOk     = ($outV | Select-String '\[PASS\]').Count -gt 0
if($aOk -and $bFailedRight -and $verifyOk){ Write-Host "`n[PASS] receipt double-post: A posted, B got INVALID_STATE, one movement" -ForegroundColor Green; exit 0 }
Write-Host "`n[FAIL] receipt double-post: A_ok=$aOk B_invalid_state=$bFailedRight verify=$verifyOk" -ForegroundColor Red
exit 1
