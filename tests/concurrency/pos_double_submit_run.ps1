<#
.SYNOPSIS
  BoutiqueOS Phase 9A - POS double submit: the same client_transaction_id from two sessions replays one sale.
...
$logA = Join-Path $res "pos_dup_A.log"; $logB = Join-Path $res "pos_dup_B.log"
...
$outB = & psql $conn -X -f (Join-Path $dir "33_pos_dup_b.sql") 2>&1
...
$bFailedRight = ($outB | Select-String '"replayed": true').Count -gt 0 -and ($outB | Select-String 'INSUFFICIENT_STOCK|ERROR').Count -eq 0
$aOk          = ($outA | Select-String 'COMMIT done').Count -gt 0
$verifyOk     = ($outV | Select-String '\[PASS\]').Count -gt 0
if($aOk -and $bFailedRight -and $verifyOk){ Write-Host "`n[PASS] pos double submit: A sold, B replayed the same sale, exactly one sale" -ForegroundColor Green; exit 0 }
Write-Host "`n[FAIL] pos double submit: A_ok=$aOk B_insufficient=$bFailedRight verify=$verifyOk" -ForegroundColor Red
exit 1
