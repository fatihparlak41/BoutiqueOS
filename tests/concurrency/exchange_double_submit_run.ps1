<#
.SYNOPSIS
  BoutiqueOS Phase 9B - exchange double submit: the same client_transaction_id from two sessions replays one exchange.
...
.EXPECTED
  A posts the exchange and holds its transaction 3 s. B starts 1 s later with the SAME
  client_transaction_id, waits on the idempotency lock and replays A's result (replayed=true).
...
$logA = Join-Path $res "xch_dup_A.log"; $logB = Join-Path $res "xch_dup_B.log"
...
Write-Host "==> session B (same client_transaction_id; must wait, then replay)" -ForegroundColor Cyan
$outB = & psql $conn -X -f (Join-Path $dir "43_return_dup_b.sql") 2>&1
...
$bFailedRight = ($outB | Select-String '"replayed": true').Count -gt 0 -and ($outB | Select-String 'OVER_RETURN|ERROR').Count -eq 0
$aOk          = ($outA | Select-String 'COMMIT done').Count -gt 0
$verifyOk     = ($outV | Select-String '\[PASS\]').Count -gt 0
if($aOk -and $bFailedRight -and $verifyOk){ Write-Host "`n[PASS] exchange double submit: A exchanged, B replayed the same exchange, exactly one return" -ForegroundColor Green; exit 0 }
Write-Host "`n[FAIL] exchange double submit: A_ok=$aOk B_right=$bFailedRight verify=$verifyOk" -ForegroundColor Red
exit 1
