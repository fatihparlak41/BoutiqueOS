<#
.SYNOPSIS
  BoutiqueOS Phase 9A - POS final-unit race through rpc_pos_complete_sale (two terminals, two client_transaction_ids).
...
$logA = Join-Path $res "pos_race_A.log"; $logB = Join-Path $res "pos_race_B.log"
...
$outB = & psql $conn -X -f (Join-Path $dir "32_pos_sale_b.sql") 2>&1
...
if($aOk -and $bFailedRight -and $verifyOk){ Write-Host "`n[PASS] pos last unit: A sold, B got INSUFFICIENT_STOCK, state consistent" -ForegroundColor Green; exit 0 }
Write-Host "`n[FAIL] pos last unit: A_ok=$aOk B_insufficient=$bFailedRight verify=$verifyOk" -ForegroundColor Red
exit 1
