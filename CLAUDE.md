...
| Faz 8A — mal kabul + iniş maliyeti motoru (`20260916090000_phase8a_landed_cost`, `20260916100000_phase8a_receipt_preview_rpc`) | **DEPLOYED TO DEV** (2026-09-15) — ek masraflar (`goods_receipt_charges`), dağıtım yöntemi (tutar/adet/eşit; manual mimaride, `NOT_IMPLEMENTED`), `rpc_goods_receipt_review` + parmak izi (`STALE_DRAFT`), POST iniş maliyetini satır/pool/deftere yazar, borç değerlemeden ayrı, ters kayıt ayrı değişmez belge (`goods_receipt_reversals`); fresh-DB 808/0, mal kabul çift-post concurrency PASS, sentetik canlı smoke PASS (fixture `ZZ E2E RECEIVING TEST` cancelled); **TLC'nin 9 taslağı ve stok/maliyet/borcu değişmedi**; KDV modellenmedi |
...
1. **Uygulanmış migration'lar.** `supabase/migrations/20260908000001` … `20260916100000` remote'ta kayıtlı; yerinde
