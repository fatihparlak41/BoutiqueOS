...
| Faz 8B+ (gerçek TLC fatura eşleme, PO, POS, e-ticaret, OCR) | **Başlamadı** — TLC katalog batch'i ve gerçek sayım bekliyor |
...
1. **Uygulanmış migration'lar.** `supabase/migrations/20260908000001` … `20260916120000` remote'ta kayıtlı; yerinde
...
fonksiyon imzaları, `RAISE` placeholder/argüman sayısı. Faz 7A itibarıyla temiz (0 hata, 0 uyarı); baseline dosyası yok, her ERROR regresyondur.
...
storage RLS politikaları yerelde de test edilir. Beklenen sayım: **808**. Çift-post yarışları: `.\tests\concurrency\count_double_post_run.ps1` ve `.\tests\concurrency\receipt_double_post_run.ps1` (sonra `db_fresh` tazele).
