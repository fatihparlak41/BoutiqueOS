...
| Faz 9B+ (iade/değişim/iptal UI, rezervasyon dönüştürme, fiş yazdırma, gerçek TLC fatura eşleme, PO, e-ticaret, OCR) | **Başlamadı** — TLC katalog batch'i ve gerçek sayım bekliyor |
...
1. **Uygulanmış migration'lar.** `supabase/migrations/20260908000001` … `20260916170000` remote'ta kayıtlı; yerinde
...
storage RLS politikaları yerelde de test edilir. Beklenen sayım: **941**. Çift-post yarışları: `.\tests\concurrency\count_double_post_run.ps1`, `.\tests\concurrency\receipt_double_post_run.ps1`, `.\tests\concurrency\pos_last_unit_run.ps1` ve `.\tests\concurrency\pos_double_submit_run.ps1` (sonra `db_fresh` tazele).
