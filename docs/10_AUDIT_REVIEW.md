...
| E-posta | invite / magiclink / recovery şablonları `token_hash` sözleşmesiyle | signup doğrulaması aynı sözleşme | `/auth/confirm` zaten `type=signup` kabul eder; `redirect_to` allowlist'e `/basvuru` eklendi (R26–R28); **dashboard "Confirm signup" şablonu bu oturumda doğrulanamadı** — bkz. docs/12 (yapılması gereken) |
...
### Ertelenen (13A)
Kart/online ödeme (Stripe/iyzico), otomatik deneme süresi, kullanım tabanlı faturalama, abonelik bitişinde otomatik askı, plan özellik kısıtları (`features` jsonb ayrılmış), e-posta bildirimleri (onay/ret e-postası; şu an sayfa yenileme), platform yöneticisi yönetimi UI'sı (`platform_admins` hâlâ SQL ile), "Confirm signup" şablonunun canlı e-posta E2E'si.
